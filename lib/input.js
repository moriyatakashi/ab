...
export class Input {
  ...
  bind(elementId, keyName) {
    if (typeof elementId !== "string" || elementId.trim() === "")
      throw new TypeError("[Input] elementId は空でない文字列である必要があります (受け取った値: " + JSON.stringify(elementId) + ")");
    if (typeof keyName !== "string" || keyName.trim() === "")
      throw new TypeError("[Input] keyName は空でない文字列である必要があります (受け取った値: " + JSON.stringify(keyName) + ")");

    const btn = document.getElementById(elementId);
    if (!btn) {
      console.warn("[Input] 要素が見つかりません: #" + elementId);
      return;
    }
    const set = (v) => () => {
      this.keys[keyName] = v;
